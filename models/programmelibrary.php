<?php
//class to manipulate data from xml file containing all programmes
class ProgrammeLibrary {
    ...
    exit();      
        ...
        ProgrammeLibrary::checkError($fileError);   
            }
    }
...
//function to get xml data from file
    public function getXmlData() {
        if (file_exists('uploads/programmes.xml')){
            $xml = simplexml_load_file('uploads/programmes.xml'); 
            //create empty array to contain programmes from xml file
            $list = [];
                foreach($xml->children()  as $programme) {
            //place each programme into the array and return it
                    $list[] = new Programme($programme->name, $programme->mood,$programme->img_path,$programme->description);
                }       
            return $list;    
        }
        else {
            exit('Failed to open programmes.xml.');
        }
    }
}